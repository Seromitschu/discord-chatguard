      ...
      if (requireDB.logs === true) {
        ...
        if (member.permissions.has(memberPerms)) return;
          ...
          const e = new EmbedBuilder().setDescription(
            `${Emojis.WARN} More than 500 characters message are not allowed in this server, ${user}.`
          );
          ...
          if (newMessage.content.length > "500") {
            ...
            if (!logChannel) return;
              ...
              const lognewMessage = await logChannel.send({
                embeds: [
                  new EmbedBuilder()
                    .setDescription(
                      `${Emojis.GUARD} <@${user.id}> has been warned for sending a more than 500 characters message.\n\`\`\`${newMessage.content}\`\`\``
                    )
              ...
              const col = await lognewMessage.createMessageComponentCollector({
                  ...
                  case "linktimeout":
                    {
                      ...
                      const timeoutEmbed = new EmbedBuilder()
                        .setTitle("Timeout")
                        .setDescription(
                          `${Emojis.WARN} You have received a timeout from \`${newMessage.guild.name}\` for sending more than 500 characters message.`
                        )
                        .setTimestamp();
                  ...
                  case "linkkick":
                    {
                      ...
                      const kickEmbed = new EmbedBuilder()
                        .setTitle("Kicked")
                        .setDescription(
                          `${Emojis.WARN} You have been kicked from \`${newMessage.guild.name}\` for sending more than 500 characters message.`
                        )
                        .setTimestamp();
                      ...
                      newMessage.member
                        ...
                        .then(() => {
                          newMessage.member.kick({
                            reason: "Sending more than 500 characters message.",
                          });
                        });
                    }
                    break;
