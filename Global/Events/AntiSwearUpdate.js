    ...
    if (requireDB.logs === true) {
      ...
      if (member.permissions.has(memberPerms)) return;
        ...
        const e = new EmbedBuilder().setDescription(
          `${Emojis.WARN} Swears are not allowed in this server, ${user}.`
        );
        const scamLinks = require("../../Database/Config/BadWords.json");
            ...
            if (!logChannel) return;
              ...
              const logMsg = await logChannel.send({
                embeds: [
                  new EmbedBuilder()
                    .setDescription(
                      `${Emojis.GUARD} <@${user.id}> has been warned for sending a swears.\n\`\`\`${newMessage.content}\`\`\``
                    )
                  ...
                  case "linktimeout":
                    {
                      ...
                      const timeoutEmbed = new EmbedBuilder()
                        .setTitle("Timeout")
                        .setDescription(
                          `${Emojis.WARN} You have received a timeout from \`${newMessage.guild.name}\` for sending swears.`
                        )
                        .setTimestamp();
                  ...
                  case "linkkick":
                    {
                      ...
                      const kickEmbed = new EmbedBuilder()
                        .setTitle("Kicked")
                        .setDescription(
                          `${Emojis.WARN} You have been kicked from \`${newMessage.guild.name}\` for sending swears.`
                        )
                        .setTimestamp();
                      ...
                      newMessage.member
                        ...
                        .then(() => {
                          newMessage.member.kick({ reason: "Sending swears." });
                        });
                    }
                    break;
                }
