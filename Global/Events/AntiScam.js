    ...
    if (requireDB.logs === true) {
      ...
      if (member.permissions.has(memberPerms)) return;
        ...
        const e = new EmbedBuilder().setDescription(
          `${Emojis.WARN} Scam Links are not allowed in this server, ${user}.`
        );
        const scamLinks = require("../../Database/Config/ScamLinks.json");
            ...
            if (!logChannel) return;
              ...
              const logMsg = await logChannel.send({
                embeds: [
                  new EmbedBuilder()
                    .setDescription(
                      `${Emojis.GUARD} <@${user.id}> has been warned for sending a scam link.\n\`\`\`${msg.content}\`\`\``
                    )
                  ...
                  case "linktimeout":
                    {
                      ...
                      const timeoutEmbed = new EmbedBuilder()
                        .setTitle("Timeout")
                        .setDescription(
                          `${Emojis.WARN} You have received a timeout from \`${msg.guild.name}\` for sending scam links.`
                        )
                        .setTimestamp();
                  ...
                  case "linkkick":
                    {
                      ...
                      const kickEmbed = new EmbedBuilder()
                        .setTitle("Kicked")
                        .setDescription(
                          `${Emojis.WARN} You have been kicked from \`${msg.guild.name}\` for sending scam links.`
                        )
                        .setTimestamp();
                      ...
                      msg.member
                        ...
                        .then(() => {
                          msg.member.kick({ reason: "Sending scam links." });
                        });
                    }
                    break;
                }
